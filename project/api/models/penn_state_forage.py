...
class PennStateForage(Document):
    """Penn State forage view identical to Penn State Diet (counts-based input).

    Inputs:
    - count_19mm, count_8mm, count_1_18mm, count_bottom (integers)
    - fdn_bromate_pct (0–100)

    Computed (cached):
    - total_count, pct_19mm, pct_8mm, pct_1_18mm, pct_bottom (0–100)
    ...
    diet: Optional[str] = Field(default=None, description="Diet or forage name/category")
    ...
    # Input and cached results
    fdn_bromate_pct: Optional[float] = Field(default=None, ge=0, le=100, description="FDN bromate (%)")
    effectiveness_factor_pct: float = Field(default=0.0, ge=0, le=100)
    fdnef_pct: float = Field(default=0.0, ge=0, le=100)

    class Settings:
        name = "penn_state_forage"
        indexes = [
    ...
    model_config = ConfigDict(
        json_schema_extra={
            "example": {
                ...
                "effectiveness_factor_pct": 27.82 + 32.04 + (1.76/2),
                "fdnef_pct": 68.5 * ((27.82 + 32.04 + (1.76/2)) / 100.0),
            }
        }
    )
    ...
    @staticmethod
    def _calc_effectiveness(p19: float, p8: float, p118: float) -> float:
        return float(p19 or 0.0) + float(p8 or 0.0) + (float(p118 or 0.0) / 2.0)
