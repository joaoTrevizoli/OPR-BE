...
router = APIRouter(
    prefix=f"{settings.API_VERSION}/trough-score",
    tags=["trough-score"],
    responses={404: {"description": "Not found"}},
)


@router.post("/", response_model=TroughScoreRead)
@auth_guard(require_admin=False, allow_read_only=False)
async def create_trough_score(request: Request, payload: TroughScoreCreate = Body(...)):
    return await ctrl.create_entry(payload)


@router.get("/", response_model=List[TroughScoreRead])
@auth_guard(require_admin=False, allow_read_only=True)
async def list_trough_scores(
    request: Request,
...
@router.get("/{entry_id}", response_model=TroughScoreRead)
@auth_guard(require_admin=False, allow_read_only=True)
async def get_trough_score(request: Request, entry_id: str):
    user: User = await get_current_user(request)
    return await ctrl.get_entry(entry_id, user=user)


@router.put("/{entry_id}", response_model=TroughScoreRead)
@auth_guard(require_admin=False, allow_read_only=False)
async def update_trough_score(request: Request, entry_id: str, updates: TroughScoreUpdate = Body(...)):
    return await ctrl.update_entry(entry_id, updates)


@router.delete("/{entry_id}")
@auth_guard(require_admin=False, allow_read_only=False)
async def delete_trough_score(request: Request, entry_id: str):
