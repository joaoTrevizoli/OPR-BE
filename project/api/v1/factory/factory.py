...
router = APIRouter(
    prefix=f"{settings.API_VERSION}/factory",
    tags=["factory"],
...
async def create_factory(request: Request, payload: FactoryCreate = Body(...)):
...
@router.get("/{entry_id}", response_model=FactoryRead)
@auth_guard(require_admin=False, allow_read_only=True)
async def get_factory(request: Request, entry_id: str):
    user: User = await get_current_user(request)
    return await ctrl.get_entry(entry_id, user=user)


@router.put("/{entry_id}", response_model=FactoryRead)
@auth_guard(require_admin=False, allow_read_only=False)
async def update_factory(request: Request, entry_id: str, updates: FactoryUpdate = Body(...)):
    return await ctrl.update_entry(entry_id, updates)


@router.delete("/{entry_id}")
@auth_guard(require_admin=False, allow_read_only=False)
async def delete_factory(request: Request, entry_id: str):
