...
class PennStateForageBase(BaseModel):
    ...
    diet: Optional[str] = Field(default=None, description="Forage/diet name/category")
...
class PennStateForageCreate(PennStateForageBase):
    pass


class PennStateForageUpdate(BaseModel):
    ...
    @classmethod
    def int_ok(cls, v):
        return PennStateForageBase._coerce_int(v)

    @field_validator('fdn_bromate_pct', mode='before')
    @classmethod
    def percent_ok(cls, v):
        return PennStateForageBase._coerce_percent(v)


class PennStateForageRead(PennStateForageBase):
...
class PennStateForageList(BaseModel):
    items: List[PennStateForageRead]
