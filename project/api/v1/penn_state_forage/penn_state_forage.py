...
from .schemas import (
    PennStateForageCreate,
    PennStateForageRead,
    PennStateForageUpdate,
)
from . import controllers as ctrl

router = APIRouter(
    prefix=f"{settings.API_VERSION}/penn-state-forage",
    tags=["penn-state-forage"],
    responses={404: {"description": "Not found"}},
)


@router.post("/", response_model=PennStateForageRead)
@auth_guard(require_admin=False, allow_read_only=False)
async def create_ps_forage(request: Request, payload: PennStateForageCreate = Body(...)):
    return await ctrl.create_entry(payload)


@router.get("/", response_model=List[PennStateForageRead])
@auth_guard(require_admin=False, allow_read_only=True)
async def list_ps_forage(
    request: Request,
...
@router.get("/{entry_id}", response_model=PennStateForageRead)
@auth_guard(require_admin=False, allow_read_only=True)
async def get_ps_forage(request: Request, entry_id: str):
    user: User = await get_current_user(request)
    return await ctrl.get_entry(entry_id, user=user)


@router.put("/{entry_id}", response_model=PennStateForageRead)
@auth_guard(require_admin=False, allow_read_only=False)
async def update_ps_forage(request: Request, entry_id: str, updates: PennStateForageUpdate = Body(...)):
    return await ctrl.update_entry(entry_id, updates)


@router.delete("/{entry_id}")
@auth_guard(require_admin=False, allow_read_only=False)
async def delete_ps_forage(request: Request, entry_id: str):
